...
import {
  ...
  VirtualAction
} from '../schedulers/VirtualTimeScheduler'

const defaultMaxFrame = 750
const completionTag = '|'
// const errorTag = '#'
const startTag = '^'
const unsubscriptionTag = '!'
// const timeTag = '-'
export class TestScheduler extends VirtualTimeScheduler {
  ...
  createTime (marbles) {
    const index = marbles.indexOf(completionTag)
    if (index === -1) {
      throw new Error(
        'marble diagram for time should have a completion marker "|"'
      )
    }
    return index * TestScheduler.frameTimeFactor
  }
  ...
  createColdObservable (marbles, values, error) {
    ...
    if (marbles.indexOf(unsubscriptionTag) !== -1) {
