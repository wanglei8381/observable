...
describe('Observable.prototype.startWith', function () {
  ...
  it('startWith(s) should prepend to a cold Observable', function () {
    var e1 = cold('---a--b--c--|')
    var e1subs = '^           !'
  ...
  it('should start with given value and raises error if source raises error', function () {
    ...
    var e1subs = '^ !'
    var expected = 'x-#'
    expectObservable(e1.startWith(defaultStartValue)).toBe(
      expected,
      defaultStartValue
    )
    expectSubscriptions(e1.subscriptions).toBe(e1subs)
  ...
  it('should accept scheduler as last argument with single value', function () {
    ...
    var e1subs = '^    !'
    var expected = 'x-a--|'
    expectObservable(e1.startWith(defaultStartValue, rxTestScheduler)).toBe(
      expected
    )
    expectSubscriptions(e1.subscriptions).toBe(e1subs)
  })
